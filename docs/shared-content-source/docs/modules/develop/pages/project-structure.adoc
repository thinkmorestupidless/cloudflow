...
[source, scala]
----
include::{cloudflow-examples-version}@docsnippets:ROOT:example$pekkostreams-scala/project/cloudflow-plugins.sbt[]
----
...
In any streamlet the default base image value is `eclipse-temurin:25-jre-alpine`. A replacement must also be Alpine-based, because the image build installs packages with `apk` and creates its user with BusyBox's `addgroup` and `adduser`. To override this base image in your cloudflow application go to its build.sbt and set up a value in settings such as:
====
...
[source, scala]
----
include::{cloudflow-examples-version}@docsnippets:ROOT:example$pekkostreams-scala/build.sbt[tag=docs-projectSetup-example]
----
