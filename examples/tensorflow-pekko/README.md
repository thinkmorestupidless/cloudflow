...
<p>
<img src="./images/tensorflow-pekko.png" width="800">
<i>Simple model serving application using TensorFlow and Pekko</i>
...
```bash
$ kubectl cloudflow configure tensorflow-pekko model-server.model=model-2/saved/1
```
...
[info]   docker.io/lightbend/tensorflow-pekko:467-26acd87-dirty
[success] Cloudflow application CR generated in /Users/myuser/lightbend-repos/cloudflow/examples/tensorflow-pekko/target/tensorflow-pekko.json
[success] Use the following command to deploy the Cloudflow application:
[success] kubectl cloudflow deploy /Users/myuser/lightbend-repos/cloudflow/examples/tensorflow-pekko/target/tensorflow-pekko.json
...
```bash
$ kubectl create ns tensorflow-pekko
```
...
Copy the models in this project to the persistent volume (NOTE: this script expects the `tensorflow-pekko` namespace to already exist):
...
```bash
$ kubectl cloudflow deploy /Users/myuser/lightbend-repos/cloudflow/examples/tensorflow-pekko/target/tensorflow-pekko.json
```
...
tensorflow-pekko   tensorflow-pekko   8-2a0f65d-dirty   2019-11-25 13:22:44 +0200 EET
```

* Check all pods are running.

```bash
$ kubectl get pods -n tensorflow-pekko
NAME                                                    READY   STATUS      RESTARTS   AGE
copy-models-wlsk5                                       0/1     Completed   0          76s
tensorflow-pekko-console-egress-6d9c8b6c44-h5t8p         1/1     Running     1          4h40m
tensorflow-pekko-model-server-7777c6c69f-klph4           1/1     Running     3          4h31m
tensorflow-pekko-wine-record-generator-fd79bd5f4-ldrwg   1/1     Running     0          4h40m
```

* Verify the application output

```bash
$ kubectl -n tensorflow-pekko logs tensorflow-pekko-console-egress-6d9c8b6c44-h5t8p
...
```bash
$ kubectl cloudflow undeploy tensorflow-pekko
```
