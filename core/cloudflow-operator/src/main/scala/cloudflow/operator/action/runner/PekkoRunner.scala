...
/** Creates the Resources that define an Pekko [[Runner]].
  */
final class PekkoRunner(pekkoRunnerDefaults: PekkoRunnerDefaults) extends Runner[Deployment] {
  import PekkoRunner._
  import pekkoRunnerDefaults._
  ...
  def appActions(app: App.Cr, labels: CloudflowLabels, ownerReferences: List[OwnerReference]): Seq[Action] = {
    app.getSpec.serviceAccount match {
      case Some(_) => Seq()
      case _ =>
        val rolePekko = pekkoRole(app.namespace, labels, ownerReferences)
        Seq(
          Action.createOrReplace(pekkoServiceAccount(app.namespace, labels, ownerReferences)),
          Action.createOrReplace(rolePekko),
          Action.createOrReplace(pekkoRoleBinding(app.namespace, rolePekko, labels, ownerReferences)))
    }
  }

  private def pekkoServiceAccount(
      namespace: String,
    ...
    def execute(client: KubernetesClient)(implicit ec: ExecutionContext): Future[Action] = {
      Future {
        client
          ...
          .patch(deployment)
        Action.log.info("Pekko deployment patched.")
        this
  ...
  def expectedPodCount(deployment: App.Deployment) = deployment.replicas.getOrElse(PekkoRunner.DefaultReplicas)

  private def pekkoRole(namespace: String, labels: CloudflowLabels, ownerReferences: List[OwnerReference]): Role = {
    new RoleBuilder()
      .withMetadata(
        new ObjectMetaBuilder()
          .withName(Name.ofPekkoRole)
          .withNamespace(namespace)
          .withLabels(labels(Name.ofPekkoRole).asJava)
          .withOwnerReferences(ownerReferences: _*)
          .build())
      .withKind("Role")
      .withRules(createPekkoClusterPolicyRule, createEventPolicyRule)
      .build()
  }

  private def pekkoRoleBinding(
      namespace: String,
      role: Role,
      labels: CloudflowLabels,
    ...
    new RoleBindingBuilder()
      .withMetadata(
        new ObjectMetaBuilder()
          .withName(Name.ofPekkoRoleBinding)
          .withNamespace(namespace)
  ...
  private val createPekkoClusterPolicyRule = {
  ...
  def resource(
    ...
    // See cloudflow.pekkostream.internal.HealthCheckFiles
