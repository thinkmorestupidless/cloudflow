...
class HttpServerSpec
  ...
  def customIngress = new PekkoServerStreamlet() {
  ...
  var port: Int = _
  var ingress: PekkoStreamlet = _
  var out: ProbeOutletTap[Data] = _

  val outlet = AvroOutlet[Data]("out", _.id.toString)
  def createDefaultIngress = new PekkoServerStreamlet() {
    val shape = StreamletShape(outlet)
    override def createLogic = HttpServerLogic.default(this, outlet)
  }

  def startIngress(ingress: PekkoStreamlet = createDefaultIngress): Unit = {
