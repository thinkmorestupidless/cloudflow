...
abstract class TestMerge extends PekkoStreamlet {
...
class JavaTestMerge(inletCount: Int) extends TestMerge {
  ...
  /** The streamlet logic for a `Merge` is fixed and merges inlet elements in the order they become available to the
    * streamlet.
    */
  override final def createLogic = new cloudflow.pekkostream.javadsl.RunnableGraphStreamletLogic(context) {
    def createRunnableGraph =
      cloudflow.pekkostream.util.javadsl.Merger.source(context, inletPorts.asJava).to(getCommittableSink(outlet))
  }
}
