...
import cloudflow.pekkostream.javadsl._
import cloudflow.pekkostream.javadsl.util.{ Either => JEither }
import cloudflow.pekkostream.scaladsl.FlowWithCommittableContext
...
object Splitter {
  ...
  /** Java API A Sink that splits elements based on a flow of type `FlowWithCommittableContext[I, Either[L, R]]`.
  ...
  def sink[I, L, R](
    ...
    pekkostream.util.scaladsl.Splitter
      .sink[I, L, R](
  ...
  def sink[I, L, R](
      ...
      context: PekkoStreamletContext): Sink[Pair[I, Committable], NotUsed] =
    org.apache.pekko.stream.javadsl.Flow
      .create[Pair[I, Committable]]()
      .map(_.toScala)
      .to(pekkostream.util.scaladsl.Splitter
...
abstract class SplitterLogic[I, L, R](
    ...
    extends pekkostream.util.scaladsl.SplitterLogic(in, left, right)(context) {

  def createFlow(): FlowWithContext[I, Committable, JEither[L, R], CommittableOffset, NotUsed]
  def flow: cloudflow.pekkostream.scaladsl.FlowWithOffsetContext[I, Either[L, R]] =
