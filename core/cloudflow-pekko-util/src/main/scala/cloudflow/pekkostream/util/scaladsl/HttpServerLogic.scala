...
object HttpServerLogic {
  ...
  final def defaultStreaming[Out](server: Server, outlet: CodecOutlet[Out])(implicit
      context: PekkoStreamletContext,
  ...
  final def defaultRoute[Out](handler: Option[RejectionHandler], writer: WritableSinkRef[Out])(implicit
      fru: FromRequestUnmarshaller[Out]) =
  ...
  final def defaultStreamingRoute[Out](writer: WritableSinkRef[Out])(implicit
      mat: Materializer,
      ec: ExecutionContext,
      fbs: FromByteStringUnmarshaller[Out],
...
/** [[cloudflow.pekkostream.ServerStreamletLogic]] for accepting HTTP requests. The HttpServerLogic requires a `Server`
  * to be passed in when it is created. You need to pass in a Server to create it
  * [[cloudflow.pekkostream.PekkoServerStreamlet]] extends [[cloudflow.pekkostream.Server]], which can be used for this
  * purpose. When you define the logic inside the streamlet, you can just pass in `this`:
  * {{{
  *  import org.apache.pekko.http.scaladsl.marshallers.sprayjson.SprayJsonSupport._
  *
  *  object TestHttpServer extends PekkoServerStreamlet {
  ...
  * }}}
  */
abstract class HttpServerLogic(server: Server)(implicit context: PekkoStreamletContext)
  ...
  protected def startServer(context: PekkoStreamletContext, route: Route, port: Int): Unit =
    Http()
