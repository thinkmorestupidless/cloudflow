...
final case class Topic(
  ...
  /** Schema resolution is done based on the information in
    * http://avro.apache.org/docs/current/spec.html#Schema+Resolution all connections should use compatible schemas
    */
  ...
  /** As protobuf is a positional language, validation is done based on the field IDs. Message compatibility
    * implementation is based on Protobuf language guide and considers message A to be compatible with message B if
    * message B contains all the ids used in message A and the fields are compatible.
    *
    * The fields are considered to be compatible if:
    *
    * Field names are the same (this is required to make sure that processing application using names to access fields
    * does not break) Field types are the same (technically, according to documentation integer types int32, uint32,
    * int64, uint64, and bool are all compatible, such change in type might break consuming applications).
    *
    * Additionally, if the type is message, we validate that the message name is the same, but do not do recursive
    * validation.
    *
    * Field in both messages have to both either have oneof_index or None. The value of the index (if exists is not
    * validated) Note Be careful when adding or removing oneof fields. If checking the value of a oneof returns
    * None/NOT_SET, it could mean that the oneof has not been set or it has been set to a field in a different version
    * of the oneof. There is no way to tell the difference, since there's no way to know if an unknown field on the wire
    * is a member of the oneof. Although technically optional fields (only type supported by proto3) is compatible with
    * repeated, it might impact consuming code, so in this implementation we consider them incompatible.
    *
    * Known limitations
    *
    * Maps usage always generate incompatible messages. The issue here is when maps are used, they generate nested
    * types, which are associated with the current message and are defined based on the maps key/value. Even if the maps
