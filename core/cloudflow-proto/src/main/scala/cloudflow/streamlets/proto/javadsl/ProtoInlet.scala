...
object ProtoInlet {
  ...
  /** Creates a ProtoInlet.
    ...
    * @param clazz
    *   The type of message that can be consumed from this inlet
    */
  ...
  /** Creates a ProtoInlet.
    ...
    * @param clazz
    *   The type of message that can be consumed from this inlet
    * @param hasUniqueGroupId
    *   Specifies if the inlet should have a unique group Id across streamlet instances. Setting hasUniqueGroupId to
    *   true, when a streamlet is scaled, will result in the inlet to receive all messsages of the topic (similar to a
    *   broadcast). Setting hasUniqueGroupId to false, when a streamlet is scaled, will result in the inlet to receive
    *   only part of the messages of the topic (partitioned).
    */
  ...
  /** Creates a ProtoInlet.
    ...
    *   only part of the messages of the topic (partitioned).
    * @param errorHandler
    *   The errorHandler, which is invoked when a message could not be deserialized.
    */
