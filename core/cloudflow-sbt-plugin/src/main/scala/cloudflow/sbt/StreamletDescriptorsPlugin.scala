...
/** Plugin that generates a json containing a Map of class name and `StreamletDescriptor` for all streamlets found in
  * this sub-project. These files are used later to insert the proper image names into the `StreamletDescriptor` and
  * `StreamletDeployment` s present in the `ApplicationDescriptor` generated by `verifyBlueprint` task.
  *
  * Note that `verifyBlueprint` being executed from the top level project sets all image names to the top level project
  * name.
  */
  ...
  private[sbt] def buildStreamletDescriptors(
    ...
    Def.task {
      val detectedStreamletDescriptors = extractResult.toTry.map {
        _.map { case (str, configDescriptor) =>
