...
object BuildAppPlugin extends AutoPlugin {
  ...
  /** Generate the Application CR from application descriptor generated by verification of blueprint and the streamlet
    * descriptors generated by the build process. We need the latter since we need the proper image name associated with
    * every streamlet descriptor and streamlet deployment.
    */
  def buildApp: Def.Initialize[Task[Unit]] = Def.task {
    ...
    val streamletClassNamesToImageNameAndId: Map[String, ImageRef] = imageToStreamletDescriptorsMaps
      .map { case (imageRef, sMap) =>
        sMap.keys.map { _ -> imageRef }.toSeq
      }
      .flatten
      .toMap
