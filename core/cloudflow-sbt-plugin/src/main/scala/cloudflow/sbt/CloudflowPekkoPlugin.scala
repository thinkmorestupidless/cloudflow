...
object CloudflowPekkoPlugin extends AutoPlugin {
  ...
  override def projectSettings =
    Seq(
      libraryDependencies ++= Vector(
        "com.lightbend.cloudflow" % "cloudflow-pekko-util_3" % (ThisProject / cloudflowVersion).value,
        "com.lightbend.cloudflow" % "cloudflow-pekko_3" % (ThisProject / cloudflowVersion).value,
        "com.lightbend.cloudflow" % "cloudflow-pekko-testkit_3" % (ThisProject / cloudflowVersion).value % "test"),
      ...
      baseDockerInstructions := {
        ...
        val pekkoEntrypointFile = (ThisProject / target).value / "cloudflow" / "pekko" / "pekko-entrypoint.sh"
        IO.write(pekkoEntrypointFile, pekkoEntrypointContent)

        val prometheusYaml = (ThisProject / target).value / "cloudflow" / "pekko" / "prometheus.yaml"
        ...
        Seq(
          ...
          Instructions.Copy(CopyFile(pekkoEntrypointFile), "/opt/pekko-entrypoint.sh"),
          ...
          Instructions.Run.shell(
            Seq(
              ...
              Seq("chmod", "a+x", "/opt/pekko-entrypoint.sh")).reduce(_ ++ Seq("&&") ++ _)),
          Instructions.User(UserInImage),
          Instructions.EntryPoint.exec(Seq("bash", "/opt/pekko-entrypoint.sh")),
          Instructions
  ...
  private lazy val pekkoEntrypointContent: String =
    scala.io.Source
      .fromResource("runtimes/pekko/pekko-entrypoint.sh", getClass().getClassLoader())
      .getLines
      .mkString("\n")

  private lazy val prometheusYamlContent =
    scala.io.Source.fromResource("runtimes/pekko/prometheus.yaml", getClass().getClassLoader()).getLines.mkString("\n")
}
