...
import cloudflow.pekkostream.internal.StreamletExecutionImpl
...
private[testkit] case class TestContext(
  ...
  private def flowWithCommittableContext[T](
      outlet: CodecOutlet[T]): cloudflow.pekkostream.scaladsl.FlowWithCommittableContext[T, T] = {
    val flow = Flow[T]
  ...
  private def seqFlowWithCommittableContext[T](outlet: CodecOutlet[T])
      : cloudflow.pekkostream.scaladsl.FlowWithCommittableContext[immutable.Seq[T], immutable.Seq[T]] = {
