...
package cloudflow.pekkostream.testkit.javadsl
...
object PekkoStreamletTestKit {
  def create(sys: ActorSystem): PekkoStreamletTestKit = PekkoStreamletTestKit(sys)
  def create(sys: ActorSystem, config: Config): PekkoStreamletTestKit = PekkoStreamletTestKit(sys, config)
}

/** Java testkit for testing pekko streamlets.
  *
  * API:
  *
  * {{{
...
final case class PekkoStreamletTestKit private[testkit] (
  ...
  /** Runs the `streamlet` using a list of `inletTaps` as the source and a list of `outletTaps` as the sink. After
    * running the streamlet it also runs the assertions.
    */
  def run[T](
      streamlet: PekkoStreamlet,
  ...
  /** Runs the `streamlet` using a list of `inletTaps` as the source and an `outletTap` as the sink. After running the
  ...
  /** Runs the `streamlet` using an `inlettap` as the source and a list of `outletTaps` as the sink. After running the
    * streamlet it also runs the assertions.
    */
  def run[T](
      streamlet: PekkoStreamlet,
