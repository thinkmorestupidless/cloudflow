...
class PekkoTransformation extends PekkoStreamlet {
  val in    = AvroInlet[Data]("in")
  val out   = AvroOutlet[Data]("out").withPartitioner(RoundRobinPartitioner)
  val shape = StreamletShape(in).withOutlets(out)

  val configurableMessage = StringConfigParameter("configurable-message", "Configurable message.", Some("pekko-original"))
  ...
  override def createLogic = new RunnableGraphStreamletLogic() {
    ...
    def flow =
      FlowWithCommittableContext[Data]
        .map { data ⇒
          data.copy(src = data.src + "-pekko", payload = msg)
        }
  }
}
