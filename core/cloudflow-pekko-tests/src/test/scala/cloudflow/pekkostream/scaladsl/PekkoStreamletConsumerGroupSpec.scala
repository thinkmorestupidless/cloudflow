...
object PekkoStreamletConsumerGroupSpec {
...
import PekkoStreamletConsumerGroupSpec._

class PekkoStreamletConsumerGroupSpec extends TestcontainersKafkaSpec(ActorSystem("test", config)) {
  import system.dispatcher

  implicit override val patienceConfig = PatienceConfig(timeout = Span(10, Seconds), interval = Span(20, Millis))

  "Pekko streamlet instances" should {
  ...
  object Generator {
    ...
    def run(testData: List[Data], outlet: String): StreamletExecution = {
      ...
      val context = new PekkoStreamletContextImpl(definition(outlet), system)
  ...
  class Generator(testData: List[Data]) extends PekkoStreamlet {
    import Generator._
  ...
  object TestReceiver {
    ...
    def run(streamletRef: String, receiver: TestReceiver, genOutlet: String): StreamletExecution = {
      ...
      val context = new PekkoStreamletContextImpl(streamletDef, system)
  ...
  class TestReceiver(sink: Sink[Data, NotUsed], instance: Int) extends PekkoStreamlet {
