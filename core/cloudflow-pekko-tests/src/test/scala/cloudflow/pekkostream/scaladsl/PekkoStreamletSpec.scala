...
class PekkoStreamletSpec extends AnyWordSpec with Matchers with BeforeAndAfterAll {
  ...
  "An PekkoStreamlet" should {
    ...
    "Verify that a call to `streamletContext` in a streamlet with no configuration parameters yields an empty config" in {
      ...
      val configTestKit = PekkoStreamletTestKit(system)
    ...
    "Be able to access VolumeMounts" in {
      ...
      object VolumeMountTestProcessor extends PekkoStreamlet {
        val out = AvroOutlet[Data]("out")
      ...
      val volumeMountTestKit =
        PekkoStreamletTestKit(system).withVolumeMounts(
  ...
  class TestProcessorWithParameters extends PekkoStreamlet {
    val in = AvroInlet[Data]("in")
  ...
  class TestIngress(source: Source[Data, NotUsed]) extends PekkoStreamlet {
    val out = AvroOutlet[Data]("out", _.id.toString)
  ...
  class TestEgress[Mat](sink: Sink[Data, Future[Seq[Data]]]) extends PekkoStreamlet {
    val in = AvroInlet[Data]("in")
    var result: Seq[Data] = _
    final override val shape = StreamletShape.withInlets(in)

    override final def createLogic = new PekkoStreamletLogic() {
      def run() =
