...
trait StreamletContext {
  ...
  /** The streamlet reference which identifies the streamlet in the blueprint. It is used in a [[Streamlet]] for logging
    * and metrics, referring back to the streamlet instance using a name recognizable by the user.
    */
  ...
  /** Get the savepoint path (topic name) from the port
    ...
    * @return
    *   the savepoint path
    * @throws TopicForPortNotFoundException
  ...
  /** The full configuration for the [[Streamlet]], containing all deployment-time configuration parameters on top of
    * the normal configuration
    */
  def config: Config = streamletDefinition.config

  /** The runtime 'bootstrap.servers' for the given topic. This is provided by the cloudflow-operator when creating the
    * streamlet's 'cloudflow.runner' configuration. A 'bootstrap.servers' will always be provided as long as a 'default'
    * Kafka cluster is defined during the install of the cloudflow-operator.
    */
  ...
  /** The subset of configuration specific to a single named instance of a streamlet.
    *
    * A [[Streamlet]] can specify the set of environment- and instance-specific configuration keys it will use during
    * runtime through [[cloudflow.streamlets.Streamlet#configParameters configParameters]]. Those keys will then be made
    * available through this configuration.
    *
  ...
  /** The path mounted for a VolumeMount request from a streamlet. In a clustered deployment, the mounted path will
    ...
    * @throws MountedPathUnavailableException
