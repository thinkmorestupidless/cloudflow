...
/** Implementation of the StreamletContext trait.
  */
@InternalApi
final class PekkoStreamletContextImpl(
    private[cloudflow] override val streamletDefinition: StreamletDefinition,
    sys: ActorSystem)
    extends PekkoStreamletContext {
  private val log = LoggerFactory.getLogger(classOf[PekkoStreamletContextImpl])
  ...
  private val StopTimeoutSetting = "cloudflow.pekko.consumer-stop-timeout"
  ...
  private[pekkostream] def shardedSourceWithContext[T, M, E](
      inlet: CodecInlet[T],
  ...
  override def ready(localMode: Boolean): Unit = {
    ...
    CoordinatedShutdown(system)
      .addTask(CoordinatedShutdown.PhaseBeforeServiceUnbind, s"pekko-streamlet-${streamletRef}-unbind") { () =>
        serviceUnbind()
      }
    CoordinatedShutdown(system)
      .addTask(CoordinatedShutdown.PhaseBeforeClusterShutdown, s"pekko-streamlet-${streamletRef}-stop") { () =>
        stop().map(_ => Done)
      }
    CoordinatedShutdown(system)
      .addTask(CoordinatedShutdown.PhaseActorSystemTerminate, s"pekko-streamlet-${streamletRef}-terminate") { () =>
        Future {
