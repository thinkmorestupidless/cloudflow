...
/** An [[PekkoStreamlet]] that can listen on a port. Using this trait instead of the [[PekkoStreamlet]] ensures that the
  ...
  */
abstract class PekkoServerStreamlet extends PekkoStreamlet with Server

/** Provides `containerPort` and a `getContainerPort()` method. It returns the TCP port that is opened on the container.
  * A [[ServerStreamletLogic]] requires an implementation of this trait (for instance an [[PekkoServerStreamlet]]) when
  * it is created.
  */
trait Server { this: PekkoStreamlet =>
  ...
  /** Returns a TCP port on the container that a streamlet can listen on.
    */
  final def containerPort: Int = ServerAttribute.containerPort(this._pekkoContext.config)
