...
/** Extends a StreamletLogic. provides access to a containerPort so that it is possible to start a TCP server inside an
  * [[PekkoStreamlet]] which will be exposed through an endpoint in Kubernetes.
  */
abstract class ServerStreamletLogic(server: Server)(implicit context: PekkoStreamletContext)
    extends PekkoStreamletLogic {
