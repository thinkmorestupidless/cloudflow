...
/** Provides an entry-point for defining the behavior of an PekkoStreamlet. Override the `run` method to implement the
  * specific logic / code that should be executed once the streamlet deployed as part of a running cloudflow
  * application. See `RunnableGraphStreamletLogic` if you just want to create a RunnableGraph.
  */
abstract class PekkoStreamletLogic(implicit val context: PekkoStreamletContext)
    extends StreamletLogic[PekkoStreamletContext] {

  override def getContext(): PekkoStreamletContext = super.getContext()

  /** This method is called when the streamlet is run. Override this method to define what the specific streamlet logic
    * should do.
    */
  def run(): Unit
  ...
  /** Java API Launch the execution of the graph.
    */
  final def runGraph[T](graph: org.apache.pekko.stream.javadsl.RunnableGraph[T]): T = graph.run(system)

  /** Signals that the streamlet is ready to process data. `signalReady` completes the
    * [[cloudflow.streamlets.StreamletExecution#ready]] future. When a streamlet is run using the testkit, a
    * [[cloudflow.streamlets.StreamletExecution]] is returned. [[cloudflow.streamlets.StreamletExecution#ready]] can be
    * used for instance to wait for a server streamlet to signal that it is ready to accept requests.
    */
  final def signalReady() = context.signalReady()

  /** The ActorSystem that will run the Pekkostreamlet.
    */
  ...
  /** The default ExecutionContext of the ActorSystem (the system dispatcher).
    */
  implicit final val executionContext: scala.concurrent.ExecutionContext = system.dispatcher
  ...
  /** This source emits `T` records together with the offset position as context, thus makes it possible to commit
    * offset positions to Kafka (as received through the `inlet`). This is useful when "at-least once delivery" is
    * desired, as each message will likely be delivered one time, but in failure cases, they can be duplicated.
    *
    * It is intended to be used with `sinkWithOffsetContext(outlet: CodecOutlet[T])` or
    * [[org.apache.pekko.kafka.scaladsl.Committer#sinkWithOffsetContext]], which both commit the offset positions that
    * accompany the records, read from this source. `sinkWithOffsetContext(outlet: CodecOutlet[T])` should be used if
    * you want to commit the offset positions after records have been written to the specified `outlet`. The `inlet`
  ...
  /** This source emits `T` records together with the committable context, thus makes it possible to commit offset
    * positions to Kafka (as received through the `inlet`). This is useful when "at-least once delivery" is desired, as
    * each message will likely be delivered one time, but in failure cases, they can be duplicated.
    *
    * It is intended to be used with `committableSink(outlet: CodecOutlet[T])`, which commits the offset positions that
    * accompany the records that are read from this source after the records have been written to the specified
    * `outlet`.
    *
    * The `inlet` specifies a [[cloudflow.streamlets.Codec]] that is used to deserialize the records read from the
  ...
  def getSourceWithOffsetContext[T](
      inlet: CodecInlet[T]): org.apache.pekko.stream.javadsl.SourceWithContext[T, CommittableOffset, _] =
    sourceWithOffsetContext(inlet).asJava

  /** Java API
    * @see
    *   [[sourceWithCommittableContext]]
    */
  def getSourceWithCommittableContext[T](
      inlet: CodecInlet[T]): org.apache.pekko.stream.javadsl.SourceWithContext[T, Committable, _] =
    context.sourceWithCommittableContext(inlet).asJava

  /** This source is designed to function the same as [[sourceWithCommittableContext]] while also leveraging Pekko Kafka
    * Cluster Sharding for stateful streaming.
    *
    * This source emits `T` records together with the committable context, thus makes it possible to commit offset
    * positions to Kafka using `committableSink(outlet: CodecOutlet[T])`.
    *
    * It is required to use this source with Pekko Cluster. This source will start up Pekko Cluster Sharding using the
    * supplied `shardEntity` and configure the kafka external shard strategy to co-locate Kafka partition consumption
    * with Pekko Cluster shards.
    *
    * @param inlet
    *   the inlet to consume messages from. The inlet specifies a [[cloudflow.streamlets.Codec]] that is used to
    *   deserialize the records read from the underlying transport.
    * @param shardEntity
  ...
  def getShardedSourceWithCommittableContext[T, M, E](
      ...
      : org.apache.pekko.stream.javadsl.SourceWithContext[T, Committable, Future[NotUsed]] =
  ...
  /** The `plainSource` emits `T` records (as received through the `inlet`).
    *
    * It has no support for committing offsets to Kafka. The `inlet` specifies a [[cloudflow.streamlets.Codec]] that
    * will be used to deserialize the records read from Kafka.
    */
  def plainSource[T](
      inlet: CodecInlet[T],
      resetPosition: ResetPosition = Latest): org.apache.pekko.stream.scaladsl.Source[T, NotUsed] =
    context.plainSource(inlet, resetPosition)

  /** Java API
    */
  def getPlainSource[T](inlet: CodecInlet[T]): org.apache.pekko.stream.javadsl.Source[T, NotUsed] = plainSource(
    inlet).asJava
  ...
  def getPlainSource[T](
      inlet: CodecInlet[T],
      resetPosition: ResetPosition): org.apache.pekko.stream.javadsl.Source[T, NotUsed] =
    plainSource(inlet, resetPosition).asJava

  /** This source is designed to function the same as [[plainSource]] while also leveraging Pekko Kafka Cluster Sharding
  ...
  def getShardedPlainSource[T, M, E](
      inlet: CodecInlet[T],
      shardEntity: Entity[M, E],
      kafkaTimeout: FiniteDuration): org.apache.pekko.stream.javadsl.Source[T, Future[NotUsed]] =
  ...
  def getShardedPlainSource[T, M, E](
      ...
      shardEntity: Entity[M, E],
      resetPosition: ResetPosition = Latest,
      kafkaTimeout: FiniteDuration = 10.seconds): org.apache.pekko.stream.javadsl.Source[T, Future[NotUsed]] =
    shardedPlainSource(inlet, shardEntity, resetPosition, kafkaTimeout).asJava

  /** Creates a sink for publishing `T` records to the outlet. The records are partitioned according to the
    * `partitioner` of the `outlet`. The `outlet` specifies a [[cloudflow.streamlets.Codec]] that will be used to
    * serialize the records that are written to Kafka.
    */
  def plainSink[T](outlet: CodecOutlet[T]): Sink[T, NotUsed] = context.plainSink(outlet)

  /** Java API
    */
  def getPlainSink[T](outlet: CodecOutlet[T]): org.apache.pekko.stream.javadsl.Sink[T, NotUsed] = plainSink(
    outlet).asJava

  /** The [[org.apache.pekko.kafka.CommitterSettings]] that have been configured from the default configuration
    * `org.apache.pekko.kafka.committer`.
    */
  ...
  /** Creates a sink for publishing records to the outlet. The records are partitioned according to the `partitioner` of
    ...
    * `outlet` specifies a [[cloudflow.streamlets.Codec]] that will be used to serialize the records that are written to
    * Kafka.
    */
  ...
  /** Creates a sink, purely for committing the offsets that have been read further upstream. Batches offsets from the
    * contexts that accompany the records, and commits these to Kafka.
    */
  ...
  /** Creates a sink, purely for committing the offsets that have been read further upstream. Batches offsets from the
    * contexts that accompany the records, and commits these to Kafka. Uses a default CommitterSettings, which is
    * configured through the default configuration in `org.apache.pekko.kafka.committer`.
    */
  ...
  /** Java API
    */
  def getCommittableSink[T](outlet: CodecOutlet[T], committerSettings: CommitterSettings)
      : org.apache.pekko.stream.javadsl.Sink[org.apache.pekko.japi.Pair[T, Committable], NotUsed] =
  ...
  /** Java API
    */
  def getCommittableSink[T](committerSettings: CommitterSettings)
      : org.apache.pekko.stream.javadsl.Sink[org.apache.pekko.japi.Pair[T, Committable], NotUsed] =
  ...
  def getSinkWithOffsetContext[T](outlet: CodecOutlet[T])
      : org.apache.pekko.stream.javadsl.Sink[org.apache.pekko.japi.Pair[T, CommittableOffset], NotUsed] =
    getSinkWithOffsetContext(outlet, defaultCommitterSettings)

  /** Java API
    */
  @deprecated("Use `getCommittableSink` instead.", "1.3.1")
  def getSinkWithOffsetContext[T](outlet: CodecOutlet[T], committerSettings: CommitterSettings)
      : org.apache.pekko.stream.javadsl.Sink[org.apache.pekko.japi.Pair[T, CommittableOffset], NotUsed] =
    committableSink[T](outlet, committerSettings).asJava.contramap { case pair => (pair.first, pair.second) }
  ...
  /** Java API
    */
  @deprecated("Use `getCommittableSink` instead.", "1.3.1")
  def getSinkWithOffsetContext[T](committerSettings: CommitterSettings)
      : org.apache.pekko.stream.javadsl.Sink[org.apache.pekko.japi.Pair[T, CommittableOffset], NotUsed] =
    committableSink[T](committerSettings).asJava.contramap { case pair => (pair.first, pair.second) }
  ...
  /** Creates a [[org.apache.pekko.stream.SinkRef SinkRef]] to write to, for the specified
    * [[cloudflow.streamlets.CodecOutlet CodeOutlet]]. The records are partitioned according to the `partitioner` of the
    * `outlet`.
    *
    * @param outlet
    *   the specified [[cloudflow.streamlets.CodecOutlet CodeOutlet]]
    * @return
    *   the [[cloudflow.pekkostream.WritableSinkRef WritebleSinkRef]] created
    */
  ...
  /** The full configuration for the [[PekkoStreamlet]], containing all deployment-time configuration parameters on top
    * of the normal configuration as loaded through ActorSystem.settings.config
    */
  ...
  /** The subset of configuration specific to a single named instance of a streamlet.
    *
    * A [[cloudflow.streamlets.Streamlet]] can specify the set of environment- and instance-specific configuration keys
    * it will use during runtime through [[cloudflow.streamlets.Streamlet#configParameters]]. Those keys will then be
    * made available through this configuration.
    */
  ...
  /** The streamlet reference which identifies the streamlet in the blueprint. It is used in a
    * [[cloudflow.streamlets.Streamlet Streamlet]] for logging and metrics, referring back to the streamlet instance
    * using a name recognizable by the user.
    */
  ...
  /** The path mounted for a VolumeMount request from a streamlet. In a clustered deployment, the mounted path will
    * correspond to the requested mount path in the [[cloudflow.streamlets.VolumeMount VolumeMount]] definition. In a
    * local environment, this path will be replaced by a local folder.
    * @param volumeMount
    *   the volumeMount declaration for which we want to obtain the mounted path.
    * @return
    *   the path where the volume is mounted.
    * @throws [[cloudflow.streamlets.MountedPathUnavailableException
