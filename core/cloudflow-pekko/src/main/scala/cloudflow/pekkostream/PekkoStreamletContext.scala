...
/** Runtime context for [[PekkoStreamlet]]s, which provides means to create
  ...
  * [[org.apache.pekko.stream.Materializer Materializer]] that will be used to run the PekkoStreamlet.
  */
trait PekkoStreamletContext extends StreamletContext {

  private[pekkostream] def sourceWithCommittableContext[T](
      inlet: CodecInlet[T]): cloudflow.pekkostream.scaladsl.SourceWithCommittableContext[T]

  private[pekkostream] def shardedSourceWithCommittableContext[T, M, E](
      inlet: CodecInlet[T],
  ...
  private[pekkostream] def sourceWithOffsetContext[T](
      inlet: CodecInlet[T]): cloudflow.pekkostream.scaladsl.SourceWithOffsetContext[T]

  private[pekkostream] def plainSource[T](inlet: CodecInlet[T], resetPosition: ResetPosition): Source[T, NotUsed]
  private[pekkostream] def plainSink[T](outlet: CodecOutlet[T]): Sink[T, NotUsed]
  private[pekkostream] def shardedPlainSource[T, M, E](
      inlet: CodecInlet[T],
      shardEntity: Entity[M, E],
      resetPosition: ResetPosition = Latest,
      kafkaTimeout: FiniteDuration = 10.seconds): Source[T, Future[NotUsed]]

  private[pekkostream] def committableSink[T](
  ...
  private[pekkostream] def sinkWithOffsetContext[T](
      outlet: CodecOutlet[T],
  ...
  /** Creates a [[org.apache.pekko.stream.SinkRef SinkRef]] to write to, for the specified
    ...
    *
    * @param outlet
    *   the specified [[cloudflow.streamlets.CodecOutlet CodecOutlet]]
    * @return
    *   the [[cloudflow.pekkostream.WritableSinkRef WritableSinkRef]] created
    */
  private[pekkostream] def sinkRef[T](outlet: CodecOutlet[T]): WritableSinkRef[T]

  /** The system in which the PekkoStreamlet will be run.
    */
  implicit def system: ActorSystem
  protected val killSwitch = KillSwitches.shared(streamletRef)

  private[pekkostream] def streamletExecution: StreamletExecution

  @InternalApi
  private[pekkostream] object Stoppers {
  ...
  /** Signals that the streamlet is ready to process data.
    *
    * When a streamlet is run using `PekkoStreamletTestkit.run`, a
    * [[cloudflow.streamlets.StreamletExecution StreamletExecution]] is returned. `signalReady` completes the
    * [[cloudflow.streamlets.StreamletExecution#ready ready]] future.
    * [[cloudflow.streamlets.StreamletExecution#ready ready]] can be used for instance to wait for a
    * [[cloudflow.pekkostream.Server Server]] to signal that it is ready to accept requests.
    *
    * @return
    *   {@code true} if and only if successfully signalled. Otherwise {@code false} .
    */
  ...
  /** Registers a callback, which is called when the streamlet is stopped. It is usually used to close resources that
    * have been created in the streamlet.
    */
  def onStop(f: () => Future[Dun]): Unit = Stoppers.add(f)

  private[pekkostream] def metricTags(): Map[String, String]
}
